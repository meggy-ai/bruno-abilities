...
class LifecycleManager:
    ...
    def get_error(self, ability_name: str) -> Optional[str]:
        """
    ...
    async def initialize_with_dependencies(
        ...
        while remaining:
            ...
            tasks = [
                self.initialize_ability(abilities[name], dependencies.get(name))
                for name in ready
            ]
            
            init_results = await asyncio.gather(*tasks, return_exceptions=True)
            
            for name, result in zip(ready, init_results):
