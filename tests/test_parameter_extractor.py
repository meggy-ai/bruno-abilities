...
def test_extract_duration():
    ...
    assert ParameterExtractor.extract_duration("1 hour 30 minutes") == timedelta(
        hours=1, minutes=30
    )
    assert ParameterExtractor.extract_duration("2h 15m 30s") == timedelta(
...
def test_extract_name_value_pairs():
    ...
    pairs = ParameterExtractor.extract_name_value_pairs("title: Meeting Notes, category: Work")
    assert pairs["title"] == "Meeting Notes"
    assert pairs["category"] == "Work"

    pairs = ParameterExtractor.extract_name_value_pairs("name=John, age=30")
